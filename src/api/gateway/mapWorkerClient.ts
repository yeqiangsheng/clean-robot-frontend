...
import { setMapWorkerDebugEvent } from './mapWorkerDebug'
...
class MapWorkerClient {
  ...
  private ensureWorker() {
    ...
    this.worker = new Worker(
      new URL('../../workers/mapSnapshotWorker.ts', import.meta.url),
      { type: 'module' },
    )
    ...
    this.worker.onmessage = (event: MessageEvent<WorkerResponse>) => {
      ...
      if (message.type === 'fetch-current-map:progress') {
        setMapWorkerDebugEvent(message.event)
        return
      }
...
const mapWorkerClient = new MapWorkerClient()

export function fetchCurrentMapFromWorker(url: string) {
  return mapWorkerClient.fetchCurrentMap(url)
}
