...
- Trial release package: `release/clean-robot-site-v0.1.0-rc.2` is the current release-candidate output path. Production dependencies should install with `npm install --omit=dev`, and the packaged site gateway should be smoke-tested on `http://127.0.0.1:4173`.
